...
- **AI 摘要**：在 GitHub Actions 上跑的話，**預設就用免費的 GitHub Models**（已內建，不用任何金鑰）。
  想換引擎可另外設（擇一）：
  - **Groq**（免費、全球可用）：https://console.groq.com → API Keys → Create。
...
- **摘要引擎（自動選，由上到下）**：`GROQ_API_KEY`（`llama-3.3-70b-versatile`）→ `GEMINI_API_KEY`（`gemini-2.0-flash`）→ `ANTHROPIC_API_KEY`（`claude-opus-4-8`）→ GitHub Models（`openai/gpt-4o-mini`，用 Actions 內建 token，免費免註冊）→ 都沒有就跳過。可在 [src/summarize.js](src/summarize.js) 調整。
